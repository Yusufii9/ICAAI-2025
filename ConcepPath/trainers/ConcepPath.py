...
from ConcepPath.dassl.engine import TRAINER_REGISTRY, TrainerX
from ConcepPath.dassl.metrics import compute_accuracy
from ConcepPath.dassl.utils import load_pretrained_weights, load_checkpoint
from ConcepPath.dassl.optim import build_optimizer, build_lr_scheduler

from ConcepPath.clip import clip, attention
from ConcepPath.clip.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
class ConcepPath(nn.Module):
    ...
    def forward(self, patch_features, label, result_fp=None, test=False):
        ...
        # B, num_patchs, _ = sim_matrix.shape
        sim_matrix = F.softmax(logit_scale * patch_features @ patch_text_features.t(), dim=1)
        if sim_matrix.dim() == 2:
        ...
        slide_features = torch.bmm(sim_matrix.transpose(1, 2), patch_features)
        B, num_patchs, embedding_len = slide_features.shape
