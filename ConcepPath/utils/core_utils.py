import numpy as np
import torch
from ConcepPath.utils.utils import *
import os
...
def test(model, loader, n_classes, test_name_list, attn_score_fp, vlm_model, test=False, return_preds=False):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    test_loss = 0.
    ...
    for batch_idx, (data, label) in enumerate(loader):
        test_name = test_name_list[batch_idx].split("/")[-1].replace(".pkl", "")
        result_fp = os.path.join(attn_score_fp, vlm_model)
        if test and not os.path.exists(result_fp):
            os.makedirs(result_fp)
        result_fp = os.path.join(result_fp, f"{test_name}.pkl")
        ...
        all_probs.append(probs)
        all_labels.append(label)
        all_preds.append(Y_hat.cpu())  # ← Store predicted class
        test_loss += losses

        patient_results[test_name] = f"is {Y_hat.item() == label.item()}, pred: {Y_hat.item()}, label: {label.item()}"

    test_loss /= len(loader)

    y_true = torch.cat(all_labels, dim=0)
    y_pred = torch.cat(all_probs, dim=0)

    acc, correct_counts, total_counts, micro_f1, macro_f1, micro_auc, macro_auc, avg_sensitivity, avg_specificity = evaluate_metrics(
        y_true=y_true, y_pred=y_pred, num_classes=n_classes
    )
    ...
    if return_preds:
        ...
        all_labels_np = y_true.cpu().numpy()
        return (
            test_loss, acc, micro_f1, macro_f1, micro_auc, macro_auc,
            avg_sensitivity, avg_specificity, patient_results,
